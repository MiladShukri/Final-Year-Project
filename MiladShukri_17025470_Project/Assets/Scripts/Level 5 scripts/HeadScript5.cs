...
public class HeadScript5 : MonoBehaviour
{
    // Sets the health of the head of the zombie, this is different from the other scenes because we are testing zombies that have more health
    public int HeadHealth = 15;
    ...
    void Update()
    {
        ...
        if (HeadHealth <= 0)
        {
            ...
            EnemyScript5.count = EnemyScript5.count + 1;
        }

        // This grabs the kill count components and updates it when the count is updated
        KillCount.GetComponent<Text>().text = "Kill Count: " + EnemyScript5.count + "/10";
